document.addEventListener('DOMContentLoaded', () => {
    ...
    const recordsTable = document.getElementById('records-tbody');

    if (searchInput && deptFilter && recordsTable) {
        const rows = recordsTable.querySelectorAll('tr');
        
        const filterTable = () => {
            const searchTerm = searchInput.value.toLowerCase();
            const deptTerm = deptFilter.value.toLowerCase();
            ...
            rows.forEach(row => {
                ...
                const matchesDept = deptTerm === '' || dept.includes(deptTerm);
